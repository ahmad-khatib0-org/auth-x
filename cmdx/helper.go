...
// Deprecated: do not use this function in commands, as it makes it impossible to
// test them. Instead, return the error.
...
// CheckResponse fatals if err is nil or the response.StatusCode does not match the
// expectedStatusCode Deprecated: do not use this function in commands, as it makes
// it impossible to test them. Instead, return the error.
...
// FormatResponse takes an object and prints a json.MarshalIdent version of it
// or fatals. Deprecated: do not use this function in commands, as it makes it
// impossible to test them. Instead, return the error.
...
// Deprecated: do not use this function in commands, as it makes it impossible
// to test them. Instead, return the error.
...
// The full matched message is passed to the callback. An error returned
// from the callback is returned by Write.
...
// Exec runs the provided cobra command with the given reader as STD_IN and
// the given args. Returns STD_OUT, STD_ERR and the error from the execution.
...
func ExecCtx(ctx context.Context, cmd *cobra.Command, stdIn io.Reader, args ...string) (string, string, error) {
	...
	// needs to be on a separate line to ensure that the ouput
	// buffers are read AFTER the command ran
...
// ExecExpectedErr is a helper that assumes a failing run from
// Exec returning ErrNoPrintButFail Returns STD_ERR.
