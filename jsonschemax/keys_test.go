...
import (
	...
	"github.com/ahmad-khatib0-org/auth-x/snapshotx"
...
func fooExtensionCompile(_ jsonschema.CompilerContext, m map[string]any) (any, error) {
...
func fooExtensionValidate(_ jsonschema.ValidationContext, _, _ any) error {
	return nil
}

func (ec *extensionConfig) EnhancePath(p Path) map[string]any {
	if ec.NotAJSONSchemaKey != "" {
		fmt.Printf("enhancing path: %s with custom property %s\n", p.Name, ec.NotAJSONSchemaKey)
		return map[string]any{ec.NotAJSONSchemaKey: p.Name}
	}
	return nil
}
