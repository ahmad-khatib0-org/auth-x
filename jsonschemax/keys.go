...
type (
	...
	PathEnhancer interface {
		EnhancePath(Path) map[string]any
	}
	TypeHint int
)
...
func makeUnique(in byName) (byName, error) {
	...
	for _, p := range in {
		...
		if fmt.Sprintf("%T", p.Type) != fmt.Sprintf("%T", p.Type) {
			return nil, errors.Errorf("multiple types %+v are not supported for path: %s", []any{p.Type, vc.Type}, p.Name)
		}
