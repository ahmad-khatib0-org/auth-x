...
func setLevel(l *logrus.Logger, o *options) {
	if o.level != nil {
		...
		var err error
		l.Level, err = logrus.ParseLevel(stringsx.Coalesce(o.c.String("log.level"), os.Getenv("LOG_LEVEL")))
		if err != nil {
...
func setFormatter(l *logrus.Logger, o *options) {
	if o.formatter != nil {
		...
		// we first have to set the formatter before we can complain about the unknown format
		var unknownFormat bool
...
func (l *Logger) ReportError(r *http.Request, code int, err error, args ...any) {
	logger := l.WithError(err).WithRequest(r).WithField("http_response", map[string]any{
		"status_code": code,
	})
